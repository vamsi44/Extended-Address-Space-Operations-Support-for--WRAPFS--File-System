...
tfm = crypto_alloc_blkcipher("ctr(aes)", 0, 0);
	...
	printk("The encrypted buffer is ");
	for(i=0;i<256;i++)
		printk("%x\n",encrypted_buffer[i]);
			...
			struct page **pagep, void **fsdata)
{
	...
	*pagep = page;
	...
	//#ifdef WRAPFS_CRYPTO
	cur_page_end_size=file->f_path.dentry->d_inode->i_size;
	//#endif
	...
	if (!PageUptodate(page)){
		...
		if((i_size_read(mapping->host)<=cur_page_end_size))
			zero_user(page,0,cur_page_end_size - i_size_read(mapping->host));
